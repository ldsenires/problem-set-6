...
constant = 0.02
x_min = min(df_jam_heavy["latitude"]) - constant
x_max = max(df_jam_heavy["latitude"]) + constant
y_min = min(df_jam_heavy["longitude"]) - constant
y_max = max(df_jam_heavy["longitude"]) + constant

# Create scatter plot
scatter_plot = alt.Chart(df_jam_heavy).mark_point().encode(
    x=alt.X("latitude:Q", scale=alt.Scale(domain=[x_min, x_max])),
    y=alt.Y("longitude:Q", scale=alt.Scale(domain=[y_min, y_max])),
